...
def track_run_metrics(session_id: str, model_id: str, run_response: Any, tools_used: list[str] = None):
    ...
    try:
        ...
        total_tokens = getattr(metrics, "total_tokens", input_tokens + output_tokens)
        
        # Agno metrics times are in seconds
        latency = getattr(metrics, "time_to_first_token", 0.0)
        total_time = getattr(metrics, "time", 0.0)
